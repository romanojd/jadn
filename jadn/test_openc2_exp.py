...
class Experimental_IP_Addr(unittest.TestCase):
...
class Experimental_IP_Net(unittest.TestCase):
    ...
    ipv4_net_api = {           # API IPv4 address target.  All API values are identical except for target name.
    ...
    ipv4_net_ser = {            # IPv4 address target serialized in type-specific string (dotted decimal) format
