...
class Codec:
    ...
    def set_mode(self, verbose_rec=False, verbose_str=False):
        ...
        def symf(fld):                      # Build symbol table field entries
            ...
            opts = fs[S_FOPT]
            if fld[FieldType] == 'Enumerated' and 'rtype' in opts:      # Generate Enumerated from a referenced type
            ...
            if 'max' in opts and opts['max'] != 1:                  # Create ArrayOf for fields with cardinality > 1
                ...
                aas = [                             # Symtab entry for dynamic type
                    ...
                    {'rtype': fs[S_FDEF][FieldType], 'min': amin, 'max': amax}  # 4: S_TOPT:  Type Options (dict)
                ]
                aa[TypeName] = _add_dtype(fs, aas)                     # Add to list of dynamically generated types
            return fs
        ...
        def sym(t):                 # Build symbol table based on encoding modes
            ...
            if t[BaseType] in ['Enumerated', 'Choice', 'Map', 'Record']:
                fx, fa = (FieldID, FieldID) if 'compact' in symval[S_TOPT] else (fx, FieldName)
            ...
            symval[S_FORMAT] = get_format_function(fchk, t[BaseType], fcvt)
            return symval
...
def _bad_index(ts, k, val):
    td = ts[S_TDEF]
    raise ValueError('%s(%s): array index %d out of bounds (%d, %d)' % (td[TypeName], td[BaseType], k, len(ts[S_FLD]), len(val)))
...
def _format(ts, val, fmtop):
    try:
        ...
        td = ts[S_TDEF]
        tn = ('%s(%s)' % (td[TypeName], td[BaseType]) if td[TypeName] else td[BaseType])
        ...
        if sv is not None:
            if 'atfield' in fopts:
                ...
                if 'max' in fopts and fopts['max'] != 1:
                    ...
                    codec.symtab[f[FieldType]][S_TOPT]['rtype'] = cftype    # Patch dynamic ArrayOf with fixed type
